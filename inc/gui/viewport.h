...
public:
    ...
    /** \brief Add a marker for drawing. Marker will be copied and be shown until markers are cleared.
     *  \return handle to marker
     */
    int addMarker(Marker marker);

    //! \brief Remove marker identied by handle.
    void removeMarker(int marker_handle);
    void clearMarkers();

    Coordinate getCoordinateForPosition(double x, double y);
